...
#if defined(NDEBUG)
# error "Valuto cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x0a749449173b2b8d7ad67a33717ded6b0cfc86988468b85716d4dea37cf2cae7");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Valuto: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Valuto Signed Message:\n";
...
{
    // Valuto: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 291 days
    nSubsidy >>= (nHeight / 840000); // Valuto: 840k blocks in ~291 days

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 5 * 30; // Valuto: 2.5 minutes
static const int64 nTargetSpacing = 30; // Valuto: 30 Seconds.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Valuto: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Valuto: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x0a749449173b2b8d7ad67a33717ded6b0cfc86988468b85716d4dea37cf2cae7");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "14/Dec/2017 The day the chicken crossed the road";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("044484710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1513288602;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2199003;
        ...
        assert(block.hashMerkleRoot == uint256("0x80ff3810ef746234c9940f026b7fb5fd573fb17c58c968699fd5ed8676bc9aa1"));
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ValutoMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ValutoMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ValutoMiner(CWallet *pwallet)
{
    printf("ValutoMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("valuto-miner");
    ...
    try { loop {
        ...
        printf("Running ValutoMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ValutoMiner, pwallet));
}
